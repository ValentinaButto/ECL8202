...
La structure du modèle est la forme fonctionnelle spécifique supposée pour la distribution de données. La forme fonctionnelle d'un modèle est l'expression mathématique représentant la manière dont les variables sont combinées pour décrire leur relation. La forme fonctionnelle dépend donc de la nature des variables impliquées (comptage, données continues, catégories etc.) et des patrons qui ont été prévus. Le but d'un modèle paramétrique est donc d'ajuster une distribution à vos données qui puisse généraliser la relation entre vos variables à tous les cas similaires à votre étude de cas.
...
La vraisemblance est une mesure de la cohérence entre les données observées et les valeurs possibles des paramètres d'un modèle. La vraisemblance est ainsi une mesure de la probabilité que les données qu'on a observées se produisent conditionnellement aux paramétrés estimés.
...
où le nombre d'essais $n = 20$, $p$ est la probabilité de germination pour la population et ${n \choose y}$ représente le nombre de façons de choisir $y$ individus parmi $n$, autrément dit combien de combinaisons différentes de $y$ éléments peuvent être choisies parmi un ensemble de $n$ éléments. Nous écrivons $f(y \vert p)$ pour préciser que cette distribution de $y$ est *conditionnelle* à une certaine valeur de $p$. 
...
Si $\hat{p}$ représente une estimation ponctuelle de la proportion de succès dans la population, il est possible de calculer un intervalle de confiance auteur de $\hat{p}$, dans le quel on peut être confiant que la vraie valeur de p se trouve. L’ intervalle de confiance est donnée par la fonction $L(p)$. Cette fonction dépende de $\hat{p}$, de la valeur critiqué associé au niveau de confiance choisi $z$ et de la taille de l'échantillon $n$
...
Si l'on compare ces estimations des paramètres avec celles obtenues à partir d'un modèle linéaire où ils sont estimés par la Méthode des Moindres Carrés Ordinaires (OLS), on observe des valeurs extrêmement similaires, voire identiques.
...
Ici, le fait d'avoir des estimations des paramètres équivalentes dans le cas des méthodes des moindres carrés ordinaires (OLS) et de la maximisation de la vraisemblance (MLE) dépend du postulat dans une régression linéaire selon lequel les erreurs du modèle sont supposées suivre une distribution normale.
